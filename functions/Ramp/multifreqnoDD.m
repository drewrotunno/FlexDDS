function stack = multifreqnoDD(stack, chan, freqlist, timelist, knownCFR)
...
%                 stack = pulseB(stack,150);         %intended for debug  
            ...
            elseif ~isup(r-1) && ~isup(r)     %down then down
                % There is an issue with the AD9910 where it can't do two
                % downward sweeps ( or upward mirror ) in a row. It'll jump
                % to the next start point. This is a working script otherwise,
